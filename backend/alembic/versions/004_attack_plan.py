...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("items", postgresql.JSONB, nullable=False, server_default=sa.text("jsonb_build_array()")),
        sa.Column("cve_report", postgresql.JSONB, nullable=True),
        sa.Column("wordlist_config", postgresql.JSONB, nullable=False, server_default=sa.text("jsonb_build_object()")),
